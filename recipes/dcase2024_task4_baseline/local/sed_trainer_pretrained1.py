...
from .lt_loss import Lt_loss,embedingsModel
...
class SEDTask4(pl.LightningModule):
    ...
    def __init__(
        ...
        for param in self.sed_master.parameters():
            param.detach_()

        for param in self.sed_student.parameters():
            param.requires_grad = False

        # instantiating losses
        self.supervised_loss = torch.nn.BCELoss()
        self.strong_unsupervised_loss = torch.nn.BCELoss()
        self.weak_unsupervised_loss = torch.nn.BCELoss()
        self.lt_loss = Lt_loss()
        ...
        self.get_weak_master_f1_seg_macro = (
            torchmetrics.classification.f_beta.MultilabelF1Score(
                len(self.encoder.labels), average="macro"
            )
        )
        ...
        self.val_buffer_sed_scores_eval_master = {}
        ...
        self.test_buffer_psds_eval_master = {
            k: pd.DataFrame() for k in test_thresholds
        }
        self.test_buffer_psds_eval_after_teacher = {
            k: pd.DataFrame() for k in test_thresholds
        }
        self.test_buffer_psds_eval_after_master = {
            k: pd.DataFrame() for k in test_thresholds
        }
        self.test_buffer_sed_scores_eval_student = {}
        self.test_buffer_sed_scores_eval_teacher = {}
        self.test_buffer_sed_scores_eval_master = {}
        self.test_buffer_sed_scores_eval_after_teacher = {}
        self.test_buffer_sed_scores_eval_after_master = {}
        self.test_buffer_sed_scores_eval_unprocessed_student = {}
        self.test_buffer_sed_scores_eval_unprocessed_teacher = {}
        self.test_buffer_sed_scores_eval_unprocessed_master = {}
        self.test_buffer_sed_scores_eval_unprocessed_after_teacher = {}
        self.test_buffer_sed_scores_eval_unprocessed_after_master = {}
        self.test_buffer_detections_thres05_student = pd.DataFrame()
        self.test_buffer_detections_thres05_teacher = pd.DataFrame()
        self.test_buffer_detections_thres05_master = pd.DataFrame()
        # self.test_buffer_detections_thres05_after_teacher = pd.DataFrame()
        # self.test_buffer_detections_thres05_after_master = pd.DataFrame()

        self.embedingsModel = embedingsModel()
    ...
    def training_step(self, batch, batch_indx,optimizer_idx):
        ...
        # for i, optimizer in enumerate(self.opt):
        #     print(f"Optimizer {i}:")
        ...
        features = self.mel_spec(audio)  #[batch_num,n_mels,time]
        ...
        #139，第二阶段 2
        if 139 < current_epoch <= 277 and optimizer_idx == 1:
            ...
            #神经网络的输出是强标签时的无监督损失
            loss_unsupervised_strong = self.strong_unsupervised_loss(
                strong_preds_student[mask_unlabeled],
                strong_preds_master[mask_unlabeled]
            )

            #神经网络的输出是弱标签时的无监督损失
            loss_unsupervised_weak = self.weak_unsupervised_loss(
                weak_preds_student[mask_unlabeled],
                weak_preds_master[mask_unlabeled]
            )

            # lt_strong_preds_master = strong_preds_master.permute(0, 2, 1)
            # # teacher模型与master模型之间的时序损失
            # lt_loss = self.lt_loss(weak_preds_student, lt_strong_preds_master)
            #
            # loss_student = loss_unsupervised_strong + loss_unsupervised_weak + 0.0000001 * lt_loss
            loss_student = loss_unsupervised_strong + loss_unsupervised_weak
            #277 5
            if current_epoch == 277:
                version = self.logger.version
                torch.save(self.sed_student.state_dict(),'exp/2024_baseline/version_{}/sed_student.pth'.format(version))

            self.log("train/student/loss_unsupervised_strong", loss_unsupervised_strong)
            self.log("train/student/loss_unsupervised_weak", loss_unsupervised_weak)
            self.log("train/step", self.scheduler[1]["scheduler"].step_num, prog_bar=True)
            self.log("train/student/loss_student", loss_student, prog_bar=True)
            self.log("train/lr", self.opt[1].param_groups[-1]["lr"], prog_bar=True)

            return loss_student
        #139,第一阶段 2
        elif current_epoch <= 139 and optimizer_idx == 0:
            ...
            weight = (
                ...
                * self.scheduler[0]["scheduler"]._get_scaling_factor()
            ...
            strong_self_sup_loss = self.selfsup_loss(
                ...
                strong_preds_master.detach()[mask_unlabeled],
            )
            #弱标签的自监督损失
            weak_self_sup_loss = self.selfsup_loss(
                weak_preds_teacher[mask_unlabeled],
                weak_preds_master.detach()[mask_unlabeled],
            )
            #总的自监督损失
            tot_self_loss = (strong_self_sup_loss + weak_self_sup_loss) * weight

            # print(strong_preds_teacher.shape)
            # print(weak_preds_teacher.shape)

            # embeddings = self.embedingsModel(embeddings)
            # print(embeddings.shape)

            # 时序损失
            #teacher模型与预训练模型之间的时序损失
            # lt_loss_weak = self.lt_loss(weak_preds_teacher, embeddings)
            #先把master模型的强标签的第二维和第三维进行转置
            lt_strong_preds_master = strong_preds_master.permute(0,2,1)
            # teacher模型与master模型之间的时序损失
            lt_loss = self.lt_loss(weak_preds_teacher, lt_strong_preds_master)


            #总的损失
            tot_loss = tot_loss_supervised + tot_self_loss + 0.0000001 * lt_loss
            # tot_loss = tot_loss_supervised + tot_self_loss


            self.log("train/teacher/loss_strong", loss_strong)
            self.log("train/teacher/loss_weak", loss_weak)
            self.log("train/teacher/tot_loss_supervised", tot_loss_supervised,prog_bar=True)
            self.log("train/step", self.scheduler[0]["scheduler"].step_num, prog_bar=True)
            self.log("train/teacher/tot_self_loss", tot_self_loss, prog_bar=True)
            self.log("train/weight", weight)
            self.log("train/teacher/strong_self_sup_loss", strong_self_sup_loss, prog_bar=True)
            self.log("train/teacher/weak_self_sup_loss", weak_self_sup_loss)
            self.log("train/lr", self.opt[0].param_groups[-1]["lr"], prog_bar=True)
            self.log("train/teacher/lt_loss", 0.0000001 * lt_loss,prog_bar=True)
            self.log("train/teacher/tot_loss",tot_loss,prog_bar=True)
            return tot_loss

        #第三阶段，学生模型生成伪标签微调教师模型 277 5
        elif current_epoch > 277 and optimizer_idx == 0:
            ...
            #教师模型的无监督损失
            loss_teacher_unsupervised_strong = self.strong_unsupervised_loss(
                strong_preds_teacher[mask_unlabeled],
                strong_preds_student[mask_unlabeled]
            )

            # 神经网络的输出是弱标签时的无监督损失
            loss_teacher_unsupervised_weak = self.weak_unsupervised_loss(
                weak_preds_teacher[mask_unlabeled],
                weak_preds_student[mask_unlabeled]
            )
            ...
            # #监督损失
            # # 强标签的监督损失
            # loss_teacher_strong = self.supervised_loss(
            #     strong_preds_teacher[strong_mask],
            #     labels[strong_mask],
            # )
            #
            # # 弱标签的监督损失
            # loss_teacher_weak = self.supervised_loss(
            #     weak_preds_teacher[weak_mask],
            #     labels_weak,
            # )

            loss_teacher_unsupervised = loss_teacher_unsupervised_weak + loss_teacher_unsupervised_strong

            # loss_teacher_supervised = loss_teacher_weak + loss_teacher_strong

            # loss_teacher_total = loss_teacher_supervised + loss_teacher_unsupervised
            #第三阶段结束，保存teacher，master两个模型的参数 357  7
            if current_epoch == 357:
                version = self.logger.version
                torch.save(self.sed_teacher.state_dict(),'exp/2024_baseline/version_{}/sed_teacher.pth'.format(version))
                torch.save(self.sed_master.state_dict(),'exp/2024_baseline/version_{}/sed_master.pth'.format(version))

            self.log("train/step", self.scheduler[0]["scheduler"].step_num, prog_bar=True)
            self.log("train/teacher/loss_teacher_unsupervised", loss_teacher_unsupervised, prog_bar=True)
            self.log("train/lr", self.opt[0].param_groups[-1]["lr"], prog_bar=True)

            return loss_teacher_unsupervised


    # def optimizer_step(self, epoch, batch_idx, optimizer, optimizer_idx,optimizer_closure, *args, **kwargs):
    #     # 根据 epoch 数选择优化器
    #     if epoch <= 5:
    #         optimizer = self.optimizers()[0]  # 前5轮使用optimizer1
    #     else:
    #         optimizer = self.optimizers()[1]  # 5轮之后使用optimizer2
    #
    ...
    def validation_step(self, batch, batch_indx):
        ...
        # prediction for teacher
        strong_preds_master, weak_preds_master = self.detect(
            mels, self.sed_master, embeddings=embeddings, classes_mask=valid_class_mask
        )
        ...
        if torch.any(mask_weak):
            ...
            loss_weak_master = self.supervised_loss(
                weak_preds_master[mask_weak], labels_weak
            )
            self.log("val/weak/teacher/loss_weak", loss_weak_teacher)
            self.log("val/weak/master/loss_weak", loss_weak_master)
            ...
            self.get_weak_master_f1_seg_macro(
                weak_preds_master[mask_weak], labels_weak.long()
            )
        ...
        if torch.any(mask_strong):
            ...
            loss_strong_master = self.supervised_loss(
                strong_preds_master[mask_strong], labels[mask_strong]
            )

            self.log("val/synth/teacher/loss_strong", loss_strong_teacher)
            self.log("val/synth/master/loss_strong", loss_strong_master)
            ...
            (
                scores_unprocessed_master_strong,
                scores_postprocessed_master_strong,
                decoded_master_strong,
            ) = batched_decode_preds(
                strong_preds_master[mask_strong],
                filenames_strong,
                self.encoder,
                median_filter=self.median_filter,
                thresholds=[],
            ...
            self.val_buffer_sed_scores_eval_master.update(
                scores_postprocessed_master_strong
            )
    ...
    def validation_epoch_end(self, outputs):
        ...
        weak_master_f1_macro = self.get_weak_master_f1_seg_macro.compute()
        # desed synth dataset
        ...
        desed_scores = {
            clip_id: self.val_buffer_sed_scores_eval_master[clip_id][keys]
            for clip_id in desed_ground_truth.keys()
        }
        psds1_sed_scores_eval_master = compute_psds_from_scores(
            desed_scores,
            desed_ground_truth,
            desed_audio_durations,
            dtc_threshold=0.7,
        ...
        intersection_f1_macro_thres05_master_sed_scores_eval = (
            sed_scores_eval.intersection_based.fscore(
                desed_scores,
                desed_ground_truth,
                threshold=0.5,
        ...
        collar_f1_macro_thres05_master_sed_scores_eval = (
            sed_scores_eval.collar_based.fscore(
                desed_scores,
                desed_ground_truth,
                threshold=0.5,
        ...
        maestro_scores_master = {
            clip_id: self.val_buffer_sed_scores_eval_master[clip_id][keys]
            for clip_id in maestro_ground_truth.keys()
        }
        segment_f1_macro_optthres_master = sed_scores_eval.segment_based.best_fscore(
            maestro_scores_master,
            maestro_ground_truth,
            maestro_audio_durations,
            segment_length=1.0,
        )[0]["macro_average"]
        segment_mauc_master = sed_scores_eval.segment_based.auroc(
            maestro_scores_master,
            maestro_ground_truth,
            maestro_audio_durations,
            segment_length=1.0,
        )[0]["mean"]
        segment_mpauc_master = sed_scores_eval.segment_based.auroc(
            maestro_scores_master,
            maestro_ground_truth,
            maestro_audio_durations,
            segment_length=1.0,
        ...
        # 计算评估指标obj_metric中的synth_metric指标
        obj_metric_synth_type = self.hparams["training"].get("obj_metric_synth_type")
        if obj_metric_synth_type is None:
            synth_metric = psds1_sed_scores_eval_teacher
        elif obj_metric_synth_type == "collar":
            synth_metric = collar_f1_macro_thres05_teacher_sed_scores_eval
        elif obj_metric_synth_type == "intersection":
            synth_metric = intersection_f1_macro_thres05_teacher_sed_scores_eval
        ...
        #obj_metric是评估sed模型的一个标准，程序会根据该值的大小来保存所谓的最好的模型
        obj_metric = torch.tensor(
            weak_teacher_f1_macro.item() + synth_metric + maestro_metric
        )
        ...
        self.log(
            "val/master/weak_f1_macro_thres05/torchmetrics", weak_master_f1_macro
        )
        self.log(
        ...
        self.log(
            "val/master/intersection_f1_macro_thres05/sed_scores_eval",
            intersection_f1_macro_thres05_master_sed_scores_eval,
        )
        self.log(
        ...
        self.log(
            "val/master/collar_f1_macro_thres05/sed_scores_eval",
            collar_f1_macro_thres05_master_sed_scores_eval,
        )
        self.log("val/teacher/psds1/sed_scores_eval", psds1_sed_scores_eval_teacher)
        self.log("val/master/psds1/sed_scores_eval", psds1_sed_scores_eval_master)
        self.log(
        ...
        self.log("val/master/segment_mpauc/sed_scores_eval", segment_mpauc_master)
        self.log(
            "val/master/segment_f1_macro_thresopt/sed_scores_eval",
            segment_f1_macro_optthres_master,
        )
        self.log("val/master/segment_mauc/sed_scores_eval", segment_mauc_master)
        self.log("val/master/segment_mpauc/sed_scores_eval", segment_mpauc_master)
        ...
        self.get_weak_master_f1_seg_macro.reset()

        return obj_metric

    def on_save_checkpoint(self, checkpoint):
        checkpoint["sed_teacher"] = self.sed_teacher.state_dict()
        checkpoint["sed_master"] = self.sed_master.state_dict()
    ...
    def test_step(self, batch, batch_indx):
        ...
        # 第三阶段训练结束后的master模型的预测结果
        strong_preds_after_master, weak_preds_after_master = self.detect(
            mels, self.sed_master, embeddings
        )

        if not self.evaluation:
            loss_strong_teacher = self.supervised_loss(strong_preds_teacher, labels)
            loss_strong_master = self.supervised_loss(strong_preds_master, labels)

            self.log("test/teacher/loss_strong", loss_strong_teacher)
            self.log("test/master/loss_strong", loss_strong_master)
        ...
        (
            ...
            self.encoder,
            median_filter=self.median_filter,
            thresholds=list(self.test_buffer_psds_eval_master.keys()) + [0.5],
        )

        self.test_buffer_sed_scores_eval_unprocessed_master.update(
            scores_unprocessed_master_strong
        )
        self.test_buffer_sed_scores_eval_master.update(
            scores_postprocessed_master_strong
        )
        for th in self.test_buffer_psds_eval_master.keys():
            self.test_buffer_psds_eval_master[th] = pd.concat(
                [self.test_buffer_psds_eval_master[th], decoded_master_strong[th]],
                ignore_index=True,
            )
        ...
        #第三阶段校正过后的teacher模型计算psds
        (
            scores_unprocessed_after_teacher_strong,
            scores_postprocessed_after_teacher_strong,
            decoded_after_teacher_strong,
        ) = batched_decode_preds(
            strong_preds_after_teacher,
            filenames,
            self.encoder,
            median_filter=self.median_filter,
            thresholds=list(self.test_buffer_psds_eval_after_teacher.keys()) + [0.5],
        )

        self.test_buffer_sed_scores_eval_unprocessed_after_teacher.update(
            scores_unprocessed_after_teacher_strong
        )
        self.test_buffer_sed_scores_eval_after_teacher.update(
            scores_postprocessed_after_teacher_strong
        )
        for th in self.test_buffer_psds_eval_after_teacher.keys():
            self.test_buffer_psds_eval_after_teacher[th] = pd.concat(
                [self.test_buffer_psds_eval_after_teacher[th], decoded_after_teacher_strong[th]],
                ignore_index=True,
            )

        # 第三阶段校正过后的master模型计算psds
        (
            scores_unprocessed_after_master_strong,
            scores_postprocessed_after_master_strong,
            decoded_after_master_strong,
        ) = batched_decode_preds(
            strong_preds_after_master,
            filenames,
            self.encoder,
            median_filter=self.median_filter,
            thresholds=list(self.test_buffer_psds_eval_after_master.keys()) + [0.5],
        )

        self.test_buffer_sed_scores_eval_unprocessed_after_master.update(
            scores_unprocessed_after_master_strong
        )
        self.test_buffer_sed_scores_eval_after_master.update(
            scores_postprocessed_after_master_strong
        )
        for th in self.test_buffer_psds_eval_after_master.keys():
            self.test_buffer_psds_eval_after_master[th] = pd.concat(
        ...
        self.test_buffer_detections_thres05_master = pd.concat(
            [self.test_buffer_detections_thres05_master, decoded_master_strong[0.5]]
        )
    ...
    def on_test_epoch_end(self):
        ...
        if self.evaluation:
            ...
            save_dir_master_unprocessed = os.path.join(
                save_dir, "master_scores", "unprocessed"
            )
            sed_scores_eval.io.write_sed_scores(
                self.test_buffer_sed_scores_eval_unprocessed_master,
                save_dir_master_unprocessed,
            )
            print(f"\nRaw scores for master saved in: {save_dir_master_unprocessed}")

            save_dir_master_postprocessed = os.path.join(
                save_dir, "master_scores", "postprocessed"
            )
            sed_scores_eval.io.write_sed_scores(
                self.test_buffer_sed_scores_eval_master,
                save_dir_master_postprocessed,
            )
            print(
                f"\nPostprocessed scores for master saved in: {save_dir_master_postprocessed}"
            )
            ...
            #计算校正过的teacher模型的psds评分
            psds1_after_teacher_psds_eval = compute_psds_from_operating_points(
                self.test_buffer_psds_eval_after_teacher,
                self.hparams["data"]["test_tsv"],
                self.hparams["data"]["test_dur"],
                dtc_threshold=0.7,
                ...
                alpha_ct=0,
                alpha_st=1,
                save_dir=os.path.join(save_dir, "afterteacher", "scenario1"),
            )

            # 计算校正过的master模型的psds评分
            psds1_after_master_psds_eval = compute_psds_from_operating_points(
                self.test_buffer_psds_eval_after_master,
                self.hparams["data"]["test_tsv"],
                self.hparams["data"]["test_dur"],
                dtc_threshold=0.7,
                ...
                alpha_ct=0,
                alpha_st=1,
                save_dir=os.path.join(save_dir, "aftermaster", "scenario1"),
            )
            ...
            psds1_master_psds_eval = compute_psds_from_operating_points(
                self.test_buffer_psds_eval_master,
                self.hparams["data"]["test_tsv"],
                self.hparams["data"]["test_dur"],
                dtc_threshold=0.7,
                ...
                alpha_ct=0,
                alpha_st=1,
                save_dir=os.path.join(save_dir, "master", "scenario1"),
            )
            psds2_master_psds_eval = compute_psds_from_operating_points(
                self.test_buffer_psds_eval_master,
                self.hparams["data"]["test_tsv"],
                self.hparams["data"]["test_dur"],
                dtc_threshold=0.1,
                ...
                alpha_ct=0.5,
                alpha_st=1,
                save_dir=os.path.join(save_dir, "master", "scenario2"),
            )
            # synth dataset
            ...
            intersection_f1_macro_thres05_master_psds_eval = (
                compute_per_intersection_macro_f1(
                    {"0.5": self.test_buffer_detections_thres05_master},
                    self.hparams["data"]["test_tsv"],
                    self.hparams["data"]["test_dur"],
                )
            )
            ...
            collar_f1_macro_thres05_master = log_sedeval_metrics(
                self.test_buffer_detections_thres05_master,
                self.hparams["data"]["test_tsv"],
                os.path.join(save_dir, "master"),
            )[0]
            ...
            desed_scores = {
                clip_id: self.test_buffer_sed_scores_eval_master[clip_id][keys]
                for clip_id in desed_ground_truth.keys()
            }
            psds1_master_sed_scores_eval = compute_psds_from_scores(
                desed_scores,
                desed_ground_truth,
                desed_audio_durations,
                dtc_threshold=0.7,
            ...
            psds2_master_sed_scores_eval = compute_psds_from_scores(
                desed_scores,
                desed_ground_truth,
                desed_audio_durations,
                dtc_threshold=0.1,
            ...
            maestro_scores_master = {
                clip_id: self.test_buffer_sed_scores_eval_master[clip_id]
                for clip_id in maestro_clip_ids
            }
            segment_length = 1.0
            ...
            segment_scores_master = _get_segment_scores_and_overlap_add(
                frame_scores=maestro_scores_master,
                audio_durations=maestro_audio_durations,
                event_classes=event_classes_maestro,
                segment_length=segment_length,
            )
            sed_scores_eval.io.write_sed_scores(
                segment_scores_master,
                os.path.join(save_dir, "master", "maestro", "postprocessed"),
            )
            ...
            segment_scores_master = {
                clip_id: scores_df[keys]
                for clip_id, scores_df in segment_scores_master.items()
            }
            ...
            results.update({
                ...
                "test/master/psds1/psds_eval": psds1_master_psds_eval,
                "test/master/psds2/psds_eval": psds2_master_psds_eval,
                "test/teacher/intersection_f1_macro_thres05/psds_eval": intersection_f1_macro_thres05_teacher_psds_eval,
                "test/master/intersection_f1_macro_thres05/psds_eval": intersection_f1_macro_thres05_master_psds_eval,
                "test/teacher/collar_f1_macro_thres05/sed_eval": collar_f1_macro_thres05_teacher,
                "test/master/collar_f1_macro_thres05/sed_eval": collar_f1_macro_thres05_master,
                ...
                "test/master/psds1/sed_scores_eval": psds1_master_sed_scores_eval,
                "test/master/psds2/sed_scores_eval": psds2_master_sed_scores_eval,
                "test/teacher/intersection_f1_macro_thres05/sed_scores_eval": intersection_f1_macro_thres05_teacher_sed_scores_eval,
                "test/master/intersection_f1_macro_thres05/sed_scores_eval": intersection_f1_macro_thres05_master_sed_scores_eval,
                "test/teacher/collar_f1_macro_thres05/sed_scores_eval": collar_f1_macro_thres05_teacher_sed_scores_eval,
                "test/master/collar_f1_macro_thres05/sed_scores_eval": collar_f1_macro_thres05_master_sed_scores_eval,
                ...
                "test/master/segment_f1_macro_thresopt/sed_scores_eval": segment_f1_macro_optthres_master,
                "test/master/segment_mauc/sed_scores_eval": segment_mauc_master,
                "test/master/segment_mpauc/sed_scores_eval": segment_mpauc_master,
                "test/student/psds1/psds_eval": psds1_student_psds_eval,
                "test/after/teacher/psds1/psds_eval":psds1_after_teacher_psds_eval,
                "test/after/master/psds1/psds_eval": psds1_after_master_psds_eval,
            })
    ...
    def configure_optimizers(self):
        optimizer1 = self.opt[0]
        optimizer2 = self.opt[1]
        schedule1 = self.scheduler[0]
        schedule2 = self.scheduler[1]
        return [
            {'optimizer': optimizer1, 'lr_scheduler': schedule1},
            {'optimizer': optimizer2, 'lr_scheduler': schedule2}
        ]
